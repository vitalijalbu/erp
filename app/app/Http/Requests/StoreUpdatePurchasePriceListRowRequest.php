...
class StoreUpdatePurchasePriceListRowRequest extends FormRequest
{
    /**
        ...
        return [
            function (Validator $validator) {
                ...
                $validated = array_merge(
                    $this->validated(), [
                        'purchase_price_list_id' => $this->purchasePriceList->id
                    ] 
                );

                $response = PurchasePriceListRow::checkOverlappingDates(auth()->user()->IDcompany, $validated);
                ...
                if($response){
                    ...
                    if(!$this->date_from && !$this->dateTo){
                        ...
                        if($purchasePriceListRow){
                            abort(400, "You cannot add this new purchase price list row because there is already one equal");
                        }
                    }
                }
            }
