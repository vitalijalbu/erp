...
export const initFetchDbBlock = () => {

	Blockly.Blocks['fetch_from_db'] = {
		...
		/** Save here the current occurencies for make a diff on newValue change  */
		oldValues_: [],
		newValues_: [],
		...
		init: function () {
			this.appendDummyInput('block')
				...
				.appendField(new Blockly.FieldDropdown([["ALL", 'all'], ["FIRST", 'first']]), 'option')
			...
			this.setOutput(true);
			this.setColour(300);
			this.setTooltip(`Execute a query and choose if return all or only one row.
					Use named bind parameter for passing user defined data to the query
				`);

			// this.addIcon(new Blockly.icons.MutatorIcon(['lists_create_with_item'], this));
			this.itemCount_ = this.newValues_.length;
		},
		...
		saveExtraState: function () {
			return {
				'newValues': this.newValues_,
				'oldValues': this.oldValues_,
		...
		loadExtraState: function (state) {
			this.newValues_ = state['newValues']
			this.oldValues_ = state['oldValues']
		...
		compose: function (topBlock) {
			...
			for (var i = 0; i < this.itemCount_; i++) {
				var input = this.getInput('ADD_' + this.newValues_[i])
				if (input) {
		...
		updateShape_: function (targetCount) {
			while (this.itemCount_ < targetCount) {
				...
				if (this.itemCount_ === 0) {
					var input = this.appendValueInput('ADD_' + this.newValues_[this.itemCount_])
					.setCheck('pair_value')
					.appendField(' Values');
				}
				else {
			...
			while (this.itemCount_ > targetCount) {
				var diff = _.difference(this.oldValues_, this.newValues_)
				diff.forEach((el) => {
					this.getInput('ADD_' + el).connection.targetBlock().dispose()
					this.removeInput('ADD_' + el);
				})
		...
		onchange: function (e) {
			if (e.type === Blockly.Events.BLOCK_CHANGE && e.element === 'field' && e.name === 'query') {

				var input = this.getFieldValue('query').trim()
				setTimeout(() => {
					this.newValues_ = (input.match(/(?<!['"]):(\w+)(?!['"])/g) || [])

					this.inputCounter = this.newValues_.length
					this.minInputs = this.inputCounter

					this.updateShape_(this.newValues_.length)
					this.oldValues_ = this.newValues_

					for (var i = 0; i < this.itemCount_; i++) {
						input = this.getInput('ADD_' + this.newValues_[i])
						if (input) {
							var targetBlock = input.connection.targetBlock()
							targetBlock.setFieldValue(this.newValues_[i], 'NAME')
