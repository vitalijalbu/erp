...
import { dataProcess } from "../hooks/helper.js";

export class ProcessesAutocomplete extends Blockly.FieldTextInput {
	/**
	...
	constructor(text, opts) {
		...
		this.loadData().then((data) => {
			if (data.length > 0) {
				ProcessesAutocomplete.DATA = Object.fromEntries(data);
			} else {
				ProcessesAutocomplete.DATA = Object.fromEntries([opts]);
			}
			try {
				let value = this.getValue();
				this.setValue(value);
				if (value && ProcessesAutocomplete.DATA[value]) {
					this.textContent_.nodeValue = ProcessesAutocomplete.DATA[value];
					this.forceRerender();
				}
			} catch (error) {
				console.log(error);
	...
	loadData() {
		if (!ProcessesAutocomplete.init) {
			ProcessesAutocomplete.init = true;
			ProcessesAutocomplete.loadPromise = new Promise((resolve, reject) => {
				dataProcess().then((data) => {
					resolve(data);
				});
			});
		}
		return ProcessesAutocomplete.loadPromise;
	}

	/**
	 * Construct a ProcessesAutocomplete from a JSON arg object.
	 * @param {!Object} options A JSON object with options (pitch).
	 * @returns {!ProcessesAutocomplete} The new field instance.
	 * @package
	 * @nocollapse
	 */
	static fromJson(options) {
		// `this` might be a subclass of ProcessesAutocomplete if that class doesn't
	...
	dropdownCreate_() {
		...
		ProcessesAutocomplete.ITEMS = ProcessesAutocomplete.DATA;
		var optionsLength = ProcessesAutocomplete.DATA.length;
		...
		for (let el in ProcessesAutocomplete.ITEMS) {
			...
			item.innerHTML = ProcessesAutocomplete.ITEMS[el];
	...
	onMouseMove(e) {
		...
		// 	var highLight = Array.from(ProcessesAutocomplete.ITEMS);
		// 	var note =
	 ...
	 */
	valueToNote(value) {
		// if (ProcessesAutocomplete.ITEMS && ProcessesAutocomplete.ITEMS.length > 0) {
		return ProcessesAutocomplete.DATA[value] ?? "";
		// } else return "";
	}
	...
	noteToValue(text) {
		...
		var i = _.map(ProcessesAutocomplete.DATA, (v, k) => v).indexOf(normalizedText);
		ProcessesAutocomplete.ITEMS = [];
		var words = ProcessesAutocomplete.ITEMS;
		var initwords = Object.values(ProcessesAutocomplete.DATA);
		...
		if (ProcessesAutocomplete.ITEMS.length == 0)
		...
		for (let el in ProcessesAutocomplete.ITEMS) {
			...
			item.addEventListener("click", (e) => {
				...
				this.setEditorValue_(
					_.findKey(ProcessesAutocomplete.DATA, (o) => o === e.target.innerHTML)
				);
				this.hide_();
			});
			item.innerHTML = ProcessesAutocomplete.ITEMS[el];
...
try {
	Blockly.fieldRegistry.register("processes_autocomplete", ProcessesAutocomplete);
} catch (e) {}
