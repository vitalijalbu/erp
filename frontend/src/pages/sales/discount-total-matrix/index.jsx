...
import { toggleDiscountTotalMatrix, getAllDiscountTotalMatrix, deleteDiscountTotalMatrix } from "@/api/sales/discount-total-matrix";
import PageActions from "@/shared/components/page-actions";
import { Modal, Space, Row, Col, Button, message, Tag } from "antd";
const { confirm } = Modal;
import { IconCopy, IconEye, IconList, IconPlayerPause, IconPlus, IconTrash } from "@tabler/icons-react";
...
const Index = () => {
    ...
    const handleDeleteRow = async (id) => {
        confirm({
            ...
            async onOk() {
                ...
                const { data, error, validationErrors } = await deleteDiscountTotalMatrix(id);
    ...
    const toggleRow = async (idRow, isDisabled) => {
        ...
        var { status, error, errorMsg } = await toggleDiscountTotalMatrix(idRow, {
            disable: !isDisabled,
        });
        if (error) {
    ...
    const handleTableChange = async (params) => {
        const result = await getAllDiscountTotalMatrix(params);
        return result.data;
    };
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            render: (text, record) =>
                record.id === null ? null : (
                    <Space.Compact>
                        <Link key={0} title="Show" href={`/sales/discount-total-matrix/${record.id}`}>
                            <Button icon={<IconEye />}>Show</Button>
                        </Link>
                        <Link key={1} title="Show" href={`/sales/discount-total-matrix/${record.id}/rows`}>
    ...
    return (
        <div className="page">
            <PageActions
                title="Discount Total Matrix"
                extra={[
                    <Link href="/sales/discount-total-matrix/create" key={0}>
