...
import {
	createPurchasesPricelist,
	updatePurchasePricelist,
	getPurchasesPricelist,
} from "@/lib/api/purchases/pricelist";
	...
	const handleSubmit = async (values) => {
		...
		try {
			const { error, data, errorMsg, validationErrors } = props?.data?.id
				? await updatePurchasePricelist(props.data.id, payload)
				: await createPurchasesPricelist(values);
			...
			if (error) {
				...
				message.success("Purchases pricelist saved successfully");
				props?.toggle?.();
				props?.reload?.();
				if (!props?.data?.id && props?.isModal === false) {
					router.push(
						!!data?.id
							? `/purchases/purchases-pricelist/${data?.id}`
							: `/purchases/purchases-pricelist`
					);
				}
			}
		} catch (error) {
