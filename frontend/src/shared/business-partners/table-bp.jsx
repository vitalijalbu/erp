...
const TableBP = (props) => {
    ...
    const handleTableChange = async (filters) => {
        try {
            const result = await getAllBP(_.merge(props.filter, filters));
    ...
    const tableColumns = [
        ...
        props?.hasActions && {
            ...
            render: (record) => (
                <Space.Compact>
                    <Link key={0} href={`/business-partners/${record.id}`} target="_blank">
                        <Button icon={<IconEye />}>
                            Show
                        </Button>
                    </Link>
                    <Button icon={<IconPencilMinus />} onClick={() => togglePopup(record)}>Edit</Button>
                </Space.Compact>
            ),
        },
    ...
    return (
        <>
            ...
            <div className="page">
                ...
                <PageActions
                	title={<TagSelection
						selected={selected} data={data} displayField="desc" callback={getBPById}
					/>}
                    extra={[
                        props.isModal ? (
                            <Button
                        ...
                        props.isModal ? (
                            ...
                            <Link target="_blank" href="/business-partners/create" key="1">
