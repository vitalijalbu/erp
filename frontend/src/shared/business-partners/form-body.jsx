...
import { Row, Col, Card, Form, Button, message, Tabs, Alert } from "antd";
...
const FormBody = (props) => {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (id) {
				response = await updateBP(id, updatedValues);
			} else {
			...
			if (error) {
				...
				// push route
				if (!id && data) {
				 	router.push(`/business-partners/${data?.IDbp}`);
				}
				setReload(reload + 1);
	...
	useEffect(() => {
		if (router.isReady) {
			...
			if (id) {
				(async () => {
					const { data, error } = await getBPById(id);
					if (!error) {
						...
						const formattedData = {
							...
							invoice_document_language_id: data?.invoice_document_language_id ?? null,
							// Roles - Shipping
							...
							shipping_document_language_id: data?.shipping_document_language_id ?? null,
							...
							purchase_document_language_id: data?.purchase_document_language_id ?? null,
						};
		...
		<>
			<div className="page">
				...
				>
					...
					{isBpBlocked && (
						<Alert
							message="Business Partner is Blocked."
							type="error"
							showIcon
						/>
					)}
