...
import {
	createDiscountTotalMatrix,
	updateDiscountTotalMatrix,
	getDiscountTotalMatrix,
} from "@/lib/api/sales/discount-total-matrix";
	...
	const handleSubmit = async (values) => {
		...
		try {
			const { error, data, errorMsg, validationErrors } = props?.data?.id
				? await updateDiscountTotalMatrix(props.data.id, payload)
				: await createDiscountTotalMatrix(values);
			...
			if (error) {
				...
				if (!props?.data?.id && props?.isModal === false) {
					router.push(
						!!data?.id
							? `/sales/discount-total-matrix/${data?.id}`
							: `/sales/discount-total-matrix`
					);
				}
			}
		} catch (error) {
