...
const FormBody = (props) => {
	...
	const handleSubmitCreate = async (values) => {
		...
		if (error) {
			...
			router.push("/configurator/routing-constraints");
		}
	};
	...
	useEffect(() => {
		(async () => {
			const response = await getConstraintTypeById("routing");
			if (response.error) {
				message.error(response.error.response.data.message);
			} else {
				console.log(response.data);
				setConstraintType(response.data);
				form.setFieldValue("constraint_type_id", response.data.id);
			}
		})();
		return () => {
	...
	return (
		<>
			<PageActions
				backUrl="/configurator/routing-constraints"
				title={
					id ? (
						<>
