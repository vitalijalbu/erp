...
const FormBody = (props) => {
	...
	const id =
		props.id || (!props.isModal && router.query.id ? router.query.id : null);
	...
	useEffect(() => {
		if (id && router.isReady) {
			console.log(data);
			setLoading(true);
			getConstraintById(id).then(({ data, error }) => {
	...
	const handleSubmitCreate = async (values) => {
		...
		if (error) {
			...
			if (!props.isModal) router.push("/configurator/pricing-constraints");
			else props.toggle();
		}
	};
