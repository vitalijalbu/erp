...
import { createBOMRuleStdProduct } from "@/api/configurator/standard-products";
...
import SelectBomConstraint from "@/shared/form-fields/other/select-bom-constraint";

const DrawerBOMRuleStdProduct = ({
	opened,
	toggle,
	data,
	id,
	...
	const handleSubmit = useCallback(
		async (values) => {
			...
			try {
				...
				({ status, error, validationErrors } = await createBOMRuleStdProduct(
					id,
					values
				));
	...
	return (
		<>
			...
			<Drawer
				...
				title={
					data
						? `Update Standard Product BOM Rule  #${data.id}`
						: `Create new Standard Product BOM Rule `
				}
			>
				<Row gutter={16}>
					<Col span={24}>
							...
							<Row gutter={16}>
								...
								<Col span={12}>
									<Form.Item
										...
										name='constraint_id'
										{...validationErrorsBag.getInputErrors("constraint_id")}
									>
										<SelectBomConstraint
											placeHolder='Search BOM Constraint'
											onChange={(value) => {
												form.setFieldValue("constraint_id", value?.id || value);
											}}
										/>
									</Form.Item>
								</Col>
...
export default DrawerBOMRuleStdProduct;
