...
export class Multiselect {
	...
	init(options) {
		...
		if (!Blockly.ContextMenuRegistry.registry.registry_.workspaceSelectAll) {
			...
			ContextMenu.registerOurContextMenu(this.useCopyPasteMenu_, this.useCopyPasteCrossTab_);
			Shortcut.unregisterShortcut();
			Shortcut.registerOurShortcut(this.useCopyPasteCrossTab_);
		}

		this.controls_ = new MultiselectControls(this.workspace_, options.multiselectIcon, this);
		...
		if (e.type === Blockly.Events.SELECTED) {
			...
			try {
				...
				this.blockSelection_.forEach((id) => {
					...
					if (block.type === blockType && typeof e.newValue !== "object") {
	...
	onKeyDown_(e) {
		if (
			e.keyCode === Blockly.utils.KeyCodes.SHIFT &&
