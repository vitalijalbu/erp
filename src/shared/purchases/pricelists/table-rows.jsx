...
const TableRows = (props) => {
	...
	const handleTableChange = async (params) => {
		...
		const result = await getAllRowsPricelist(id, params);
		// Use lodash's maxBy to get the object with the greatest 'order'
		const higherIdObject = _.maxBy(result.data?.data, "order");
		// Extract the 'order' value directly
		const higherOrder = higherIdObject ? parseInt(higherIdObject.order) : 0;
		form.setFieldValue("order", higherOrder + 1);
		...
		confirm({
			...
			async onOk() {
				try {
					...
					const { data, error } = await deleteRowPricelist(id, idRow);
					if (error) {
	...
	const handleSubmit = async (values) => {
		...
		var { status, error, validationErrors } = await addRowPricelist(id, values);
		if (error) {
			if (validationErrors) {
	...
	const tableColumns = [
		...
		},
		{
			...
			dataIndex: "price",
			render: (price) => (
				price && <Text>{price} {props.currency?.symbol}</Text>
			),
		},
		{
			title: "Qty (to)",
		...
		},
		{
			title: "Width (to)",
			key: "width",
			render: ({width}) => (width && <Text>{width} mm</Text>)
		},
		{
			title: "Date from",
			key: "date_from",
						...
						<Row gutter={16}>
							<Col span={6}>
								<Form.Item
									label='Order'
									name='order'
									{...validationErrorsBag.getInputErrors("order")}
								>
									<InputNumber min={0} />
								</Form.Item>
							</Col>
							...
							<Col span={6}>
								<Form.Item
									label='Width (to)'
									name='width'
									{...validationErrorsBag.getInputErrors("width")}
								>
									<InputNumber
										placeholder='0.00'
										min={0}
							...
							<Col span={6}>
								<Form.Item
									label='Price'
									name='price'
									{...validationErrorsBag.getInputErrors("price")}
								>
									<InputNumber
										addonAfter={props.currency?.symbol}
										placeholder='0.00'
										min={0}
										decimalSeparator='.'
									/>
							...
							<Col span={6}>
								<Form.Item
									label='Class'
									name='class'
									{...validationErrorsBag.getInputErrors("class")}
								>
									<Select options={[]} />
								</Form.Item>
							</Col>
