...
import { getAllRows, deleteRow, addRow, toggleRow } from "@/api/sales/discount-total-matrix";
...
const TableRows = (props) => {
	...
	const handleFieldValueChange = (changedFields) => {
		...
		const otherFields = ["item_id", "item_group_id", "item_subfamily_id"].filter(
	...
	const tableColumns = [
		...
		},
		{
			title: "Bp",
			key: "bp_desc",
			// render: (record) => record.bp_id && `${record.bp_id} - ${record.bp_desc}`,
			render: (record) => record.bp_id && `${record.bp_desc}`,
		},
		{
			title: "Bp Group",
			key: "bp_desc",
			render: (record) => record.bp_group_id && `${record.bp_group_name}`,
		},
		{
			title: "Item",
			key: "item_item",
			render: (record) => record.item_item && `${record.item_item} - ${record.item_desc}`,
		},
		{
			title: "Service Item",
			key: "service_item_item",
			render: (record) =>
				record.service_item_id &&
				`${record.service_item_item} - ${record.service_item_desc}`,
		},
		{
			title: "Product type",
			key: "group_desc",
			render: (record) => record.item_group && `${record.item_group} - ${record.group_desc}`,
		},
		{
			title: "Item group",
						...
						<Row gutter={16}>
							...
							<Col span={6}>
								<Form.Item
									...
									name="bp_group_id"
									{...validationErrorsBag.getInputErrors("bp_group_id")}
								>
									<SelectBPGroup
										onChange={(value) =>
											form.setFieldValue("bp_group_id", value)
										}
									/>
								</Form.Item>
							</Col>
							...
							<Col span={6}>
								<Form.Item
									...
									{...validationErrorsBag.getInputErrors("service_item_id")}
								>
