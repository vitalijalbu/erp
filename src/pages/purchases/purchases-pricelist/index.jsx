...
import {
	getAllPurchasesPricelist,
	deletePurchasePricelist,
	togglePurchasesPricelist,
} from "@/api/purchases/pricelist";
...
const Index = () => {
	...
	const handleDeleteRow = async (id) => {
		confirm({
			...
			async onOk() {
				...
				const { data, error, validationErrors } = await deletePurchasePricelist(
					id
				);
		...
		const toggleRow = async (idRow, isDisabled) => {
			...
			var { status, error, errorMsg } = await togglePurchasesPricelist(idRow, {
				disable: !isDisabled,
			});
			if (error) {
	...
	const handleTableChange = async (params) => {
		const result = await getAllPurchasesPricelist(params);
		return result.data;
	};
				...
				record.id === null ? null : (
					<Space.Compact>
						<Link
							key={0}
							title='Show'
							href={`/purchases/purchases-pricelist/${record.id}`}
						>
						...
						<Link
							key={1}
							title='Show Rows'
							href={`/purchases/purchases-pricelist/${record.id}/rows`}
						>
						...
						<Button
							...
							icon={<IconTrash />}
							danger
							loading={_.isEqual(loadingAction, ['delete', record.id])}
	...
	return (
		<>
		 {popup && <ModalClone isSale={false} opened={popup} data={selected} toggle={() => setPopup(!popup)} reload={() => setReload(reload + 1)}/>}
		 {popupChangePrices && <ModalChangePrices isSale={false} opened={popupChangePrices} data={selected} toggle={() => setPopupChangePrices(!popupChangePrices)} reload={() => setReload(reload + 1)}/>}
		<div className='page'>
			<PageActions
				title='Purchases Pricelists'
				extra={[
					<Link
						href='/purchases/purchases-pricelist/create'
						key={0}
					>
						<Button
							type='primary'
