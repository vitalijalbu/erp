...
import { dataOperators } from "../hooks/helper.js";

export class OperatorAutocomplete extends Blockly.FieldTextInput {
	/**
	...
	constructor(text, opts) {
		...
		this.loadData().then((data) => {
			if (data.length > 0) {
				OperatorAutocomplete.DATA = Object.fromEntries(data);
			} else {
				OperatorAutocomplete.DATA = Object.fromEntries([opts]);
			}
			try {
				let value = this.getValue();
				this.setValue(value);
				if(value && OperatorAutocomplete.DATA[value]) {
					this.textContent_.nodeValue = OperatorAutocomplete.DATA[value];
					this.forceRerender();
				}
			} catch (error) {
				console.log(error);
	...
	loadData() {
		if(!OperatorAutocomplete.init) {
			OperatorAutocomplete.init = true;
			OperatorAutocomplete.loadPromise = new Promise((resolve, reject) => {
				dataOperators().then((data) => {
					resolve(data);
				});
			});
		}
		return OperatorAutocomplete.loadPromise;
	}

	/**
	 * Construct a OperatorAutocomplete from a JSON arg object.
	 * @param {!Object} options A JSON object with options (pitch).
	 * @returns {!OperatorAutocomplete} The new field instance.
	 * @package
	 * @nocollapse
	 */
	static fromJson(options) {
		// `this` might be a subclass of OperatorAutocomplete if that class doesn't
	...
	dropdownCreate_() {
		...
		OperatorAutocomplete.ITEMS = OperatorAutocomplete.DATA;
		var optionsLength = OperatorAutocomplete.DATA.length;
		...
		for (let el in OperatorAutocomplete.ITEMS) {
			...
			item.innerHTML = OperatorAutocomplete.ITEMS[el];
	...
	onMouseMove(e) {
		...
		// 	var highLight = Array.from(OperatorAutocomplete.ITEMS);
		// 	var note =
	 ...
	 */
	valueToNote(value) {
		// if (OperatorAutocomplete.ITEMS && OperatorAutocomplete.ITEMS.length > 0) {
		return OperatorAutocomplete.DATA[value] ?? "";
		// } else return "";
	}
	...
	noteToValue(text) {
		...
		var i = _.map(OperatorAutocomplete.DATA, (v, k) => v).indexOf(normalizedText);
		OperatorAutocomplete.ITEMS = [];
		var words = OperatorAutocomplete.ITEMS;
		var initwords = Object.values(OperatorAutocomplete.DATA);
		...
		if (OperatorAutocomplete.ITEMS.length == 0)
		...
		for (let el in OperatorAutocomplete.ITEMS) {
			...
			item.addEventListener("click", (e) => {
				...
				this.setEditorValue_(
					_.findKey(OperatorAutocomplete.DATA, (o) => o === e.target.innerHTML)
				);
				this.hide_();
			});
			item.innerHTML = OperatorAutocomplete.ITEMS[el];
...
try {
	Blockly.fieldRegistry.register("operator_autocomplete", OperatorAutocomplete);
} catch (e) {}
