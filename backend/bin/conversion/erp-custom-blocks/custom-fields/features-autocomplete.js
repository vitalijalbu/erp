...
export class FeaturesAutocomplete extends Blockly.FieldTextInput {
	/**
	...
	constructor(text, opts) {
		...
		dataFeatures().then((data) => {
			if (data.length > 0) {
				FeaturesAutocomplete.DATA  = Object.fromEntries(data);
			} else {
				FeaturesAutocomplete.DATA  = Object.fromEntries([opts]);
			}
			try {
				this.setValue(text)
			} catch (error) {
	...
	/**
	 * Construct a FeaturesAutocomplete from a JSON arg object.
	 * @param {!Object} options A JSON object with options (pitch).
	 * @returns {!FeaturesAutocomplete} The new field instance.
	 * @package
	 * @nocollapse
	 */
	static fromJson(options) {
		// `this` might be a subclass of FeaturesAutocomplete if that class doesn't
	...
	dropdownCreate_() {
		...
		FeaturesAutocomplete.ITEMS = FeaturesAutocomplete.DATA;
		var optionsLength = FeaturesAutocomplete.DATA.length;
		...
		for (let el in FeaturesAutocomplete.ITEMS) {
			...
			item.innerHTML = FeaturesAutocomplete.ITEMS[el];
	...
	onMouseMove(e) {
		...
		// 	var highLight = Array.from(FeaturesAutocomplete.ITEMS);
		// 	var note =
	 ...
	 */
	valueToNote(value) {
		// if (FeaturesAutocomplete.ITEMS && FeaturesAutocomplete.ITEMS.length > 0) {
		return FeaturesAutocomplete.DATA[value] ?? "";
		// } else return "";
	}
	...
	noteToValue(text) {
		...
		// const i = FeaturesAutocomplete.DATA.indexOf(normalizedText);
		// return i > -1 ? i : undefined;

		var normalizedText = text.trim();
		var i = _.map(FeaturesAutocomplete.DATA, (v, k) => v).indexOf(normalizedText);
		FeaturesAutocomplete.ITEMS = [];
		var words = FeaturesAutocomplete.ITEMS;
		var initwords = Object.values(FeaturesAutocomplete.DATA);
		...
		if (FeaturesAutocomplete.ITEMS.length == 0)
		...
		for (let el in FeaturesAutocomplete.ITEMS) {
			...
			item.addEventListener("click", (e) => {
				...
				this.setEditorValue_(
					_.findKey(FeaturesAutocomplete.DATA, (o) => o === e.target.innerHTML)
				);
				this.hide_();
			});
			item.innerHTML = FeaturesAutocomplete.ITEMS[el];
...
try{
	Blockly.fieldRegistry.register("features_autocomplete", FeaturesAutocomplete);
}
catch(e){}
